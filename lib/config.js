...
ConfigFile.prototype.getProfile = function (profileName, cb) {
    ...
    this.loadLocalSettings().then(function (data){
       if(!profileName) defaultProfileNameScoped = data;
    });
        ...
        .then(function (profiles) {
            ...
            if (!profileName) {
                profileName = Object.keys(profiles).length === 1
                    ...
                    : defaultProfileNameScoped ||  'default';
            }
            
            var profile = profiles[defaultProfileNameScoped];

            if (!profile)
                throw new Cli.error.notFound('Profile `' + defaultProfileNameScoped
                    + '` not found');
...
ConfigFile.prototype.setProfile = function (profileName, profileData, cb) {
    ...
    this.loadLocalSettings().then(function (data){
        if(!profileName) profileNameScoped = data || 'default';
    });
    var promise = this.load()
        .then(function (profiles) {
            return (profiles[profileNameScoped] = profileData);
        });
